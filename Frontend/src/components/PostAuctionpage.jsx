...
const PostAuction = () => {
  ...
  const uploadImages = async (files) => {
    try {
      ...
      const response = await axios.post(
        "https://online-auction-platform-4qje.onrender.com/upload",
        formdata,
        {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('https://online-auction-platform-4qje.onrender.com/postAuction', formDataToSend, {
        headers: {
          Authorization: `Bearer ${token}`,
